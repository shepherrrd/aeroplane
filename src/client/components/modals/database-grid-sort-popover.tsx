...
function optionClass(active: boolean) {
  return `flex w-full items-center justify-between px-3 py-2 text-left text-sm transition ${
  ...
  return (
    <div className="absolute left-0 top-full z-30 mt-2 w-[360px] border border-zinc-700 bg-zinc-950 shadow-[0_22px_70px_rgba(0,0,0,0.45)]">
      <div className="border-b border-zinc-800 px-4 py-3">
        <div className="text-sm font-semibold text-zinc-100">Sort records</div>
        <div className="mt-1 text-xs text-zinc-500">Choose one column and direction.</div>
      </div>
      <div className="grid grid-cols-[1fr_116px] gap-3 p-3">
      ...
      {sort ? (
        <div className="border-t border-zinc-800 p-3">
          <button type="button" className="text-sm text-zinc-400 hover:text-white" onClick={() => onSortChange(null)}>
            Clear sort
          </button>
        </div>
      ) : null}
